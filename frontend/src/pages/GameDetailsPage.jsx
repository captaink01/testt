  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-950 via-blue-900 to-indigo-900">
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        ...
        {/* Main Game Details Glass Card - Made noticeably smaller/tighter */}
        <div className="bg-white/20 backdrop-blur-3xl border border-white/40 rounded-3xl shadow-2xl ring-1 ring-white/20 p-6 md:p-8 transition-all duration-500">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
            ...
            <span className={`px-6 py-3 rounded-full text-lg font-bold tracking-wide backdrop-blur-sm ${
          ...
          {game.description && (
            <div className="mb-8 bg-white/10 backdrop-blur-sm border border-white/30 rounded-2xl p-6">
              <h3 className="text-xl font-black text-white mb-3">Description</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-10">
            ...
            <div className="flex items-center gap-4 bg-white/10 backdrop-blur-sm border border-white/30 rounded-2xl p-5">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">{formatDate(game.date)}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 bg-white/10 backdrop-blur-sm border border-white/30 rounded-2xl p-5">
              ...
              <div>
                ...
                <p className={`text-2xl font-black ${game.current_players >= game.players_needed ? 'text-green-300' : 'text-blue-300'}`}>
            ...
            <div className="flex items-center gap-4 bg-white/10 backdrop-blur-sm border border-white/30 rounded-2xl p-5">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">{game.creator_name}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-5 items-center justify-center">
            {isCreator ? (
              ...
              >
                {actionLoading ? 'Joining...' : isFull ? 'Game Full' : game.status !== 'open' ? 'Game Closed' : 'Join Game'}
              </button>
            )}
          </div>
        </div>

        {/* Participants List - Also made tighter */}
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-white/70 text-sm font-medium">
                        Joined {new Date(participant.joined_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                      </p>
                    </div>
                  </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 transition-all duration-300">
          <div className="bg-white/25 backdrop-blur-3xl border border-white/40 rounded-3xl shadow-2xl ring-1 ring-white/20 p-8 md:p-10 max-w-md w-full mx-4 scale-100 transition-transform duration-300">
            <h3 className="text-3xl font-black text-white mb-4 text-center">Delete Game?</h3>
