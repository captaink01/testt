...
const MyGamesPage = () => {
  ...
  const [activeTab, setActiveTab] = useState(isOrganizer() ? 'created' : 'joined');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-950 via-blue-900 to-indigo-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 pt-24 pb-16">
        ...
        {/* Tabs - Only show Created tab for organizers */}
          ...
          <div className="flex border-b border-white/30">
            {isOrganizer() && (
              <button
                ...
                className={`flex-1 px-6 py-4 font-bold text-lg ${activeTab === 'created'
            ...
            <button
              ...
              className={`flex-1 px-6 py-4 font-bold text-lg ${activeTab === 'joined'
                  ? 'text-white border-b-2 border-white'
                  : 'text-white/70 hover:text-white'
                } transition-all duration-300`}
            >
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex flex-col gap-2">
                    <span className={`px-4 py-2 rounded-full text-sm font-bold ${game.status === 'open'
                    ...
                    {/* Show approval status for organizer's created games */}
                    {activeTab === 'created' && game.approval_status && (
                      <span className={`px-4 py-2 rounded-full text-xs font-bold text-center ${game.approval_status === 'approved' ? 'bg-green-500/30 text-green-200 border border-green-400/50' :
                          game.approval_status === 'rejected' ? 'bg-red-500/30 text-red-200 border border-red-400/50' :
                            'bg-yellow-500/30 text-yellow-200 border border-yellow-400/50'
                        } backdrop-blur`}>
                        {game.approval_status === 'approved' ? '✓ Approved' :
                          game.approval_status === 'rejected' ? '✗ Rejected' :
                            '⏳ Pending'}
                      </span>
                    )}
                  </div>
